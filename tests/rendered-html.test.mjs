...
test("renders the fifteen-chart Pulse Charts shell", async () => {
...
test("ships all fifteen chart snapshots", async () => {
  ...
  assert.equal(data.charts.length, 15);
  ...
  assert.equal(data.charts.reduce((total, chart) => total + chart.songs.length, 0), 480);
  ...
  assert.equal(evergreenCharts.length, 6);

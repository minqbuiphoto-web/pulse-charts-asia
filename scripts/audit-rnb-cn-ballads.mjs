...
const classics = JSON.parse(fs.readFileSync(new URL("../app/charts-classics.json", import.meta.url), "utf8"));
const rnb = JSON.parse(fs.readFileSync(new URL("../app/charts-rnb.json", import.meta.url), "utf8"));
const charts = [...classics.charts.filter((chart) => chart.id.startsWith("cn-ballad-evergreen-")), ...rnb.charts];
...
fs.writeFileSync(new URL("../.audit-rnb-cn-ballads.json", import.meta.url), JSON.stringify(results, null, 2));
console.log(`Wrote ${results.length} audited songs across 7 charts.`);
